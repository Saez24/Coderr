...
async function changeReviewFilterProfile(element) {
    ...
    if (currentUser.type == "business") {
        await setReviewsForBusinessUser(currentUser.user.pk);
        document.getElementById("business_review_list").innerHTML = getReviewWLinkTemplateList(currentReviews)
    } else if (currentUser.type == "customer") {
        await setReviewsForCustomerUser(currentUser.user.pk);
...
async function updateBusinessProfile(formData) {
    ...
    let resp = await patchData(PROFILE_URL + currentUser.user.pk + "/", formData)

    if (resp.ok) {
        let userResp = await getData(PROFILE_URL + resp.data.user.pk + "/")
